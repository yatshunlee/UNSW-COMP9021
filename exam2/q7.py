...
def find_word_horizontally(grid, word):
    ...
    for (i, j) in starts:
        ...
        for k in range(len(word)):
            if j+k >= len(grid[i]) or grid[i][j+k] != word[k]: # only difference
                found = False
                break
        if found:
            return (i+1, j+1)
...
def find_word_vertically(grid, word):
    ...
    for (i, j) in starts:
        found = True
        for k in range(len(word)):
            if i+k >= len(grid) or grid[i+k][j] != word[k]: # only difference
                found = False
                break
        if found:
            return (i+1, j+1)
...
def find_word_diagonally(grid, word):
    ...
    for (i, j) in starts:
        found = True
        for k in range(len(word)):
            if i+k >= len(grid) or j+k >= len(grid[0]) or grid[i+k][j+k] != word[k]: # only difference
                found = False
                break
        if found:
            return (i+1, j+1)
